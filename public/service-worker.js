...
const CACHE_VERSION = 'v12'; // cache-first app shell navigation + skipWaiting on install
...
async function handleNavigation() {
  ...
  // ---- Cache-first shell: 只要曾经加载过 PWA，立即返回已缓存的 index，不因弱网/误判 onLine 卡住 ----
  // 联网时在后台静默更新 CacheStorage，下次打开或刷新即拿到新版本。
  if (shell) {
      ...
      fetch('/index.html', { cache: 'no-store' })
        .then(async function (r) {
          ...
          try {
            ...
            var clean = stripRedirect(r);
            await c.put('/index.html', clean.clone());
            await c.put('/', clean.clone());
          } catch (e) { /* ignore */ }
        })
        .catch(function () { /* ignore */ });
    }
    console.log('[SW] Serving cached app shell (cache-first)');
